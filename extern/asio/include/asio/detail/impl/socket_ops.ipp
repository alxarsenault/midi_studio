    ...
    asio::error_code& ec, size_t& bytes_transferred)
{
  for (;;)
  {
    ...
    if (bytes >= 0)
    {
      bytes_transferred = (size_t)bytes;
      return true;
    }
...
#else // defined(ASIO_WINDOWS) || defined(__CYGWIN__)
  const char* result = ::inet_ntop(af, src, dest, static_cast<socklen_t>(length));
  get_last_error(ec, true);
  if (result == 0 && !ec)
    ec = asio::error::invalid_argument;
    ...
    unsigned long* scope_id, asio::error_code& ec)
{
  ...
  if (if_name != 0)
  {
    ...
    memcpy(src_buf, src, (size_t)(if_name - src));
  ...
  if (result > 0 && is_v6 && scope_id)
  {
    ...
    *scope_id = 0;
    if (if_name != 0)
    {
      ...
      if (*scope_id == 0)
        *scope_id = (unsigned long)atoi(if_name + 1);
    }
  }
  return result;
#endif // defined(ASIO_WINDOWS) || defined(__CYGWIN__)
...
int gethostname(char* name, int namelen, asio::error_code& ec)
{
  ...
  int result = ::gethostname(name, (size_t)namelen);
  get_last_error(ec, result != 0);
  return result;
#endif // defined(ASIO_WINDOWS_RUNTIME)
}
...
# endif
  ...
  {
    u_short_type port = host_to_network_short((asio::detail::u_short_type)atoi(service));
...
# endif
  ...
  clear_last_error();
  int error = ::getnameinfo(static_cast<const socket_addr_type*>(addr),
      (socklen_t)addrlen, host, (socklen_t)hostlen, serv, (socklen_t)servlen, flags);
  return ec = translate_addrinfo_error(error);
#endif
}
